...
/* It acts as a handler for notifications when mDNS discovers peers. */
type discoveryNotifee struct {
	/* h holds a reference to the libp2p host, needed for connecting to peers */
	h host.Host
	/* connectionAttempts tracks ongoing connection attempts to avoid duplicates */
	connectionAttempts map[peer.ID]time.Time
	/* mutex for safe concurrent access to the connectionAttempts map */
	mutex sync.Mutex
}
